...
## Using `[Glyph]` Attribute

In some designs, `enum` types serve a **configuration role** rather than directly identifying glyphs. In these cases, an enum member represents a command, action, or UI affordance, while the actual glyph comes from a *different* enum generated by the `GlyphProvider`.
...
This MAUI example shows a command bar that accepts a *generic* enum and turns each member into a button. The enum passed to `Configure<T>()` is **not a glyph enum**. Instead, each member must *declare* which glyph it maps to.
 ...
 ![Configurable Command Bar](https://raw.githubusercontent.com/IVSoftware/IVSoftware.GlyphProvider/master/IVSoftware.GlyphProvider.Portable/README/img/configuration-demo.png)
